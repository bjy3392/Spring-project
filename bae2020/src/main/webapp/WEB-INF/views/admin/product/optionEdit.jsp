...
<head>
	...
	<script>
		...
		function cancelEdit(idx,code){
			...
			$("#span_"+idx).html("<button type='button' class='w3-round-xlarge' id='btn_str' onclick='editOption("+idx+",\""+code+"\")'>편집</button>"+
		...
		function viewImageShow(option){
			var url = "${contextPath}/admin/viewImageShow?code="+option+"&classify=opt";
	                            ...
	                            <c:forEach var="vo" items="${vos }" varStatus="status">
		                          	<tr>
		                          		<form id="myform_${status.count}" method="post"  enctype="multipart/form-data">
			                              	...
			                              	<span id="fileSpan_${status.count}"><button type="button" class="w3-round-xlarge" id="btn_str" onclick="viewImageShow('${vo.option_code}')">미리보기</button></span></td>
			                              	<td>
			                              		<span id="span_${status.count}"><button type="button" class="w3-round-xlarge" id="btn_str" onclick="editOption(${status.count},'${vo.option_code}')">편집</button>
